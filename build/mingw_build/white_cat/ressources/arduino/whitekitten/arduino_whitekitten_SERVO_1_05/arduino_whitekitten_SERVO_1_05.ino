...
SoftwareServo myservo[total_pin+1];
int servo_val[total_pin+1];
...
void setup()
{
  ...
  ///definit les pulsation des servo
  myservo[i].setMinimumPulse(544); //en microseconde
  myservo[i].setMaximumPulse(2400); //en microseconde
...
void inputOutput_config()
{
  for(int i=2; i<=total_pin; i++){
   if(digital_is_output[i]==1&&allow_servo_write[i]==0)
   {
     ...
     {myservo[i].detach();}
   pinMode(i,OUTPUT); } 
   else if (digital_is_output[i]==1&&allow_servo_write[i]==1)
    {
    if(myservo[i].attached())
     {myservo[i].detach();}
     myservo[i].attach(i);}
...
void whitecat_to_pwm()
{
  for(int i=2; i<=last_io; i++)    // saute les 2 premiers tx/rx ...
  {
    ...
    else if (allow_pwm_write[i]==0&&allow_servo_write[i]==1)
    { 
      valeur_pwm[i]=byte(inString[i+3]-1);    // attention, caracteres PW/, puis 2 premiers canaux tx/rx, puis valeurs valides !!
      servo_val[i]=  map(valeur_pwm[i],0,254,0,160);
